"""Benchmark single-kernel CUTLASS DualGemm swiglu7 vs torch eager / torch.compile.
...
CUTLASS implementation (this file): a single fused kernel via
cutlass::gemm::device::DualGemm. Both half-GEMMs (A @ W_gate.T and
A @ W_linear.T) run in the same threadblock sharing A's smem stages;
their accumulators stay in registers and a custom Swiglu7Combine epilogue
functor combines them and writes only D — no intermediate (M, N/2)
buffer ever touches HBM.

Compare-with-this:
  eager   : torch.nn.functional.linear(A, B) followed by separate pointwise
            ops (multiple kernel launches, one (M, N) intermediate).
  compile : torch.compile(eager_ref, mode="max-autotune") — Inductor may
            fuse the pointwise tail with Triton, but the matmul itself
            still emits a full (M, N) bf16 intermediate to HBM that the
            tail kernel reads back.
  cutlass : single DualGemm kernel on sm_120 (Ampere TensorOp via backward
            compatibility). No HBM intermediate.

Usage
-----
    python examples/99_evt_demo/swiglu7_epi_one_stage_pybind_benchmark.py
    python examples/99_evt_demo/swiglu7_epi_one_stage_pybind_benchmark.py \\
        --shapes "2048,4096,4096;4096,8192,4096" --iters 50
    python examples/99_evt_demo/swiglu7_epi_one_stage_pybind_benchmark.py --no-compile
"""
...
def build_extension(verbose: bool = False, arch: str = "120"):
    ...
    return load(
        name="swiglu7_epi_one_stage_torch_ext",
        sources=[os.path.join(HERE, "swiglu7_epi_one_stage_torch_ext.cu")],
        extra_include_paths=[
            os.path.join(CUTLASS_ROOT, "include"),
            os.path.join(CUTLASS_ROOT, "tools", "util", "include"),
            HERE,                                  # swiglu7_combine.h
        ],
...
def main() -> None:
    ...
    print(f"Building CUTLASS DualGemm swiglu7 extension (arch=sm_{args.arch}) ...",
          flush=True)
    ...
    # Runs once per shape with a deterministic seed so failures reproduce;
    # reports absolute error stats and a pass/fail verdict against a
    # relative-error tolerance suited to bf16 (default 1e-2). torch.compile
    # vs eager check is also done on the first shape only (compile JIT is
    # per-shape).
    # ------------------------------------------------------------------
    ...
    print(f"\n[swiglu7 single-kernel DualGemm — bf16, alpha={ALPHA}, limit={LIMIT}]")
    print(header)
    print("-" * len(header))
